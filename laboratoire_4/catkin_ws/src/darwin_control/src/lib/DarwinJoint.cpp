...
DarwinJoint::DarwinJoint(std::string joint_name, ros::NodeHandle* node, int rate)
{
	...
	joint_name_command_ << "/darwin/" << darwin_joint_name_.c_str() << "_position_controller/command";
	joint_name_state_ << "/darwin/" << darwin_joint_name_.c_str() << "_position_controller/state";
